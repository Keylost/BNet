...
bool trainDataCollection::loadImagesFromLst(const char* fileName, int width, int height)
{
	...
	while(fscanf(fp, "%s %d\n", path, &curClass) != EOF)
	{
		...
		{
			for(int i =0;i<4;i++) dt->outputs[i] = 0.0;
			dt->outputs[4] = 1.0;
			//цвета
			if(curClass == 5) //black
			{
			...
			else if(curClass == 6) //red
			{
				dt->outputs[5] = 1.0;
				dt->outputs[6] = 0.0;
				dt->outputs[7] = 0.0;
			}
			else if(curClass == 7) //green
			{
				dt->outputs[5] = 0.0;
				dt->outputs[6] = 0.0;
				dt->outputs[7] = 1.0;
			}
			else if(curClass == 8) //yellow
			{
				dt->outputs[5] = 0.0;
				dt->outputs[6] = 1.0;
				dt->outputs[7] = 0.0;
			}
			else if(curClass == 9) //yellow + red
			{
				dt->outputs[5] = 1.0;
				dt->outputs[6] = 1.0;
				dt->outputs[7] = 0.0;
